...
### Observation Space
Data is progressively disclosed — the environment starts blank and variables expand based on the agent's investigation:
...
### Reward Function
Dense reward logic — scores are awarded at every step, not just at the end:
- `+0.05` to `+0.18` for correct investigative actions in the right order
- `+0.15` to `+0.65` for correct remediation actions
- `-0.10` to `-0.40` penalties for red herrings, wrong versions, or premature reruns
- All rewards strictly bounded to `[0.01, 0.99]`

---

## Setup & Usage

### 1. Local Validation & Inference
...
Live at: https://huggingface.co/spaces/Aparna05/oncall-hero
